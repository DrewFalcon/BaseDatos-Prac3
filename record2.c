...
BookRecord* record_read(FILE* f, long int offset) {
        ...
        if (remaining > 0) {
                ...
                if (delim) {
                        ...
                        if (printed_len >= sizeof(rec->printedBy))
                                printed_len = sizeof(rec->printedBy) - 1;
                        if (printed_len > 0)
                                memcpy(rec->printedBy, buf + title_len, printed_len);
                        rec->printedBy[printed_len] = '\0';
                } else {
                        /* no delimiter found: put all into title and keep printedBy empty */
                        size_t copy_len = remaining;
                        if (copy_len >= sizeof(rec->title))
                                copy_len = sizeof(rec->title) - 1;
